...
private _side = [_this, "SIDE", civilian] call DJOTraffic_fnc_GetParamValue;
private _possibleVehicles = [_this, "VEHICLES", ["C_Offroad_01_F", "C_Offroad_01_repair_F", "C_Quadbike_01_F", "C_Hatchback_01_F", "C_Hatchback_01_sport_F", "C_SUV_01_F", "C_Van_01_transport_F", "C_Van_01_box_F", "C_Van_01_fuel_F"]] call DJOTraffic_fnc_GetParamValue;
private _vehicleCount = [_this, "VEHICLES_COUNT", 10] call DJOTraffic_fnc_GetParamValue;
private _maxGroupsCount = [_this, "MAX_GROUPS_COUNT", 0] call DJOTraffic_fnc_GetParamValue;
private _minSpawnDistance = [_this, "MIN_SPAWN_DISTANCE", 800] call DJOTraffic_fnc_GetParamValue;
private _maxSpawnDistance = [_this, "MAX_SPAWN_DISTANCE", 1200] call DJOTraffic_fnc_GetParamValue;
private _minSkill = [_this, "MIN_SKILL", 0.3] call DJOTraffic_fnc_GetParamValue;
private _maxSkill = [_this, "MAX_SKILL", 0.7] call DJOTraffic_fnc_GetParamValue;
private _areaMarkerName = [_this, "AREA_MARKER", ""] call DJOTraffic_fnc_GetParamValue;
private _hideAreaMarker = [_this, "HIDE_AREA_MARKER", true] call DJOTraffic_fnc_GetParamValue;
private _fnc_onUnitCreating = [_this, "ON_UNIT_CREATING", { true }] call DJOTraffic_fnc_GetParamValue;
private _fnc_onUnitCreated = [_this, "ON_UNIT_CREATED", {}] call DJOTraffic_fnc_GetParamValue;
private _fnc_onUnitRemoving = [_this, "ON_UNIT_REMOVING", {}] call DJOTraffic_fnc_GetParamValue;
private _fnc_onSpawnVehicleObsolete = [_this, "ON_SPAWN_CALLBACK", {}] call DJOTraffic_fnc_GetParamValue;
private _fnc_onRemoveVehicleObsolete = [_this, "ON_REMOVE_CALLBACK", {}] call DJOTraffic_fnc_GetParamValue;
private _debug = [_this, "DEBUG", false] call DJOTraffic_fnc_GetParamValue;
...
DJOTraffic_instanceIndex = DJOTraffic_instanceIndex + 1;
_currentInstanceIndex = DJOTraffic_instanceIndex;

DJOTraffic_areaMarkerNames set [_currentInstanceIndex, _areaMarkerName];
DJOTraffic_edgeRoadsUseful set [_currentInstanceIndex, false];
DJOTraffic_roadSegments set [_currentInstanceIndex, []];
...
[] spawn DJOTraffic_fnc_FindEdgeRoads;
waitUntil { sleep 1; (DJOTraffic_edgeRoadsUseful select _currentInstanceIndex) };
sleep 5;
...
while {true} do {
    ...
    if (_areaMarkerName == "") then {
    	...
    	
    	{
	    	...
	    	private _playerCalculatedShare = [_farPos, _maxSpawnDistance, _areaMarkerName] call DJOTraffic_fnc_CalculatePlayerMarkerCoverage;
    ...
    _deletedVehiclesCount = 0;
	{
        ...
        if (_keepVehicle) then {
            ...
            [_debugMarkerName] call DJOTraffic_fnc_DeleteDebugMarkerAllClients;
    ...
    if (count _activeVehicles < _calculatedMaxVehicleCount || { _undamagedVehiclesCount < _calculatedMaxVehicleCount && count _activeVehicles < _maxGroupsCount}) then {
        ...
        _spawnSegment = [_currentInstanceIndex, _allPlayerPositions, _minDistance, _maxSpawnDistance, _activeVehicles] call DJOTraffic_fnc_FindSpawnSegment;
        ...
        if (str _spawnSegment != """NULL""") then {
            ...
            private _allRoadSegments = DJOTraffic_roadSegments select _currentInstanceIndex;
            switch (_trafficLocation) do {
                case 0: { _roadSegments = (getPos (DJOTraffic_edgeBottomLeftRoads select _currentInstanceIndex)) nearRoads 100; };
                case 1: { _roadSegments = (getPos (DJOTraffic_edgeTopLeftRoads select _currentInstanceIndex)) nearRoads 100; };
                case 2: { _roadSegments = (getPos (DJOTraffic_edgeTopRightRoads select _currentInstanceIndex)) nearRoads 100; };
                case 3: { _roadSegments = (getPos (DJOTraffic_edgeBottomRightRoads select _currentInstanceIndex)) nearRoads 100; };
	        ...
	        if (_areaMarkerName == "") then {
	            ...
	            _destinationPos = [getPos _spawnSegment, _destinationPos] call DJOTraffic_fnc_GetPosThisIsland;
	            ...
	            /*
			    if (isNil "DJOTraffic_LineMarkerNo") then { DJOTraffic_LineMarkerNo = 1 };
			    private _marker = createMarker ["DJOTraffic_LineMarker_" + str DJOTraffic_LineMarkerNo, _destinationPos];
			    _marker setMarkerShape "ICON";
			    _marker setMarkerType "hd_dot";
			    _marker setMarkerColor "ColorGreen";
			    DJOTraffic_LineMarkerNo = DJOTraffic_LineMarkerNo + 1;
				*/
	        }
	        else {
	            _destinationSegment = selectRandom _roadSegments;
			...
			if (_goOnWithSpawn && { _vehicleClassName != "" } && { !_userMessedUp }) then {
	            ...
	            // Name vehicle
	            sleep random 0.1;
	            if (isNil "DJOTraffic_CurrentEntityNo") then {
	                DJOTraffic_CurrentEntityNo = 0
	            };
	            
	            _currentEntityNo = DJOTraffic_CurrentEntityNo;
	            DJOTraffic_CurrentEntityNo = DJOTraffic_CurrentEntityNo + 1;
	            
	            _vehicleVarName = "DJOTraffic_Entity_" + str _currentEntityNo;
	            ...
	            _debugMarkerName = "DJOTraffic_DebugMarker" + str _currentEntityNo;
	            
	            // Start vehicle
	            [_currentInstanceIndex, _vehicle, _areaMarkerName, _destinationPos, _debug] spawn DJOTraffic_fnc_MoveVehicle;
	    ...
	    for "_i" from 1 to _sleepSeconds do {
	        {
	            ...
	            [_debugMarkerName, getPos (_vehicle), "mil_dot", _debugMarkerColor, "Traffic"] call DJOTraffic_fnc_SetDebugMarkerAllClients;
