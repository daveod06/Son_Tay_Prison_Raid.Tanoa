...
_unitClasses = [_this, "UNIT_CLASSES", ["C_man_1", "C_man_1_1_F", "C_man_1_2_F", "C_man_1_3_F", "C_man_polo_1_F", "C_man_polo_1_F_afro", "C_man_polo_1_F_euro", "C_man_polo_1_F_asia", "C_man_polo_2_F", "C_man_polo_2_F_afro", "C_man_polo_2_F_euro", "C_man_polo_2_F_asia", "C_man_polo_3_F", "C_man_polo_3_F_afro", "C_man_polo_3_F_euro", "C_man_polo_3_F_asia", "C_man_polo_4_F", "C_man_polo_4_F_afro", "C_man_polo_4_F_euro", "C_man_polo_4_F_asia", "C_man_polo_5_F", "C_man_polo_5_F_afro", "C_man_polo_5_F_euro", "C_man_polo_5_F_asia", "C_man_polo_6_F", "C_man_polo_6_F_afro", "C_man_polo_6_F_euro", "C_man_polo_6_F_asia", "C_man_p_fugitive_F", "C_man_p_fugitive_F_afro", "C_man_p_fugitive_F_euro", "C_man_p_fugitive_F_asia", "C_man_p_beggar_F", "C_man_p_beggar_F_afro", "C_man_p_beggar_F_euro", "C_man_p_beggar_F_asia", "C_man_w_worker_F", "C_scientist_F", "C_man_hunter_1_F", "C_man_p_shorts_1_F", "C_man_p_shorts_1_F_afro", "C_man_p_shorts_1_F_euro", "C_man_p_shorts_1_F_asia", "C_man_shorts_1_F", "C_man_shorts_1_F_afro", "C_man_shorts_1_F_euro", "C_man_shorts_1_F_asia", "C_man_shorts_2_F", "C_man_shorts_2_F_afro", "C_man_shorts_2_F_euro", "C_man_shorts_2_F_asia", "C_man_shorts_3_F", "C_man_shorts_3_F_afro", "C_man_shorts_3_F_euro", "C_man_shorts_3_F_asia", "C_man_shorts_4_F", "C_man_shorts_4_F_afro", "C_man_shorts_4_F_euro", "C_man_shorts_4_F_asia", "C_Orestes", "C_Nikos", "C_Nikos_aged"]] call DJOCivilians_fnc_GetParamValue;
_unitsPerBuilding = [_this, "UNITS_PER_BUILDING", 0.1] call DJOCivilians_fnc_GetParamValue;
_maxGroupsCount = [_this, "MAX_GROUPS_COUNT", 100] call DJOCivilians_fnc_GetParamValue;
_minSpawnDistance = [_this, "MIN_SPAWN_DISTANCE", 100] call DJOCivilians_fnc_GetParamValue;
_maxSpawnDistance = [_this, "MAX_SPAWN_DISTANCE", 500] call DJOCivilians_fnc_GetParamValue;
_blackListMarkers = [_this, "BLACKLIST_MARKERS", []] call DJOCivilians_fnc_GetParamValue;
_hideBlacklistMarkers = [_this, "HIDE_BLACKLIST_MARKERS", true] call DJOCivilians_fnc_GetParamValue;
_fnc_OnSpawnCallback = [_this, "ON_UNIT_SPAWNED_CALLBACK", {}] call DJOCivilians_fnc_GetParamValue;
_fnc_OnRemoveCallback = [_this, "ON_UNIT_REMOVE_CALLBACK", { true }] call DJOCivilians_fnc_GetParamValue;
_debug = [_this, "DEBUG", false] call DJOCivilians_fnc_GetParamValue;
...
_side = DJOCivilians_SIDE;
_minSkill = DJOCivilians_MINSKILL;
_maxSkill = DJOCivilians_MAXSKILL;
...
_spawnUnit = {
	...
	_pos = [_minSpawnDistance, _playerBuildings, _blackListMarkers] call DJOCivilians_fnc_FindSpawnPosition;
	_unit = objNull;
	...
	if (count _pos > 0) then {
		...
		DJOCivilians_INSTANCE_NO = DJOCivilians_INSTANCE_NO + 1;
		_unit setVehicleVarName "ENGIMA_CIVILIAN_UNIT_" + str (DJOCivilians_INSTANCE_NO);
...
while { true } do {
	...
	_playerBuildings = [_allPlayerPositions, _maxSpawnDistance, _blackListMarkers] call DJOCivilians_fnc_GetPlayerBuildings;
	...
	if (count _civilianItems < _unitsCount) then {
		...
		if (!isNull _unit) then {
			...
			_civilianItems pushBack [_unit, "CITIZEN", [], getPos _unit, false, time, random 1 < DJOCivilians_RUNNINGCHANCE];
		};
	...
	_civilianItemsTemp = [];
	{
		...
		if (_tooClose) then {
			...
			if (!_removeUnit) then {
				...
				[vehicleVarName _civilian] call DJOCivilians_fnc_DeleteDebugMarkerAllClients;
	...
	
	{
		...
		if (_isMoving && _lastPos distance getPos _unit < 1) then {
			_isMoving = false;
			_nextActionTime = time + random DJOCivilians_MAXWAITINGTIME;
		...
		if (!_isMoving && time > _nextActionTime) then {
			...
			_destPos = [_unit, _blackListMarkers, _maxSpawnDistance] call DJOCivilians_fnc_FindDestinationPosition;
			if (count _destPos > 0) then {
				...
				_isMoving = true;
				_isRunning = random 1 < DJOCivilians_RUNNINGCHANCE;
		...
		if (_debug) then {
			[vehicleVarName _unit, getPos _unit, "mil_dot", "ColorWhite", "Civ"] call DJOCivilians_fnc_SetDebugMarkerAllClients;
		};
