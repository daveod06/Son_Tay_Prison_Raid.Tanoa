...
if (random 100 > 50) then {
	...
	while { count _buildings > 0 && !_positionFound && _tries < 10 } do {
		...
		_buildingPosCount = [_building] call DJOCivilians_fnc_CountPositionsInBuilding;
		
		if (_buildingPosCount > 0) then {
			if (!([getPos _building, _blackListMarkers] call DJOCivilians_fnc_PositionInsideBlackMarker)) then {
...
else 
{
	...
	while { !_positionFound && _tries < 10 } do {
		...
		if (!isOnRoad _pos && !surfaceIsWater _pos && !([_pos, _blackListMarkers] call DJOCivilians_fnc_PositionInsideBlackMarker)) then {
			_foundPosition = _pos;
			_positionFound = true;
		};
	};
