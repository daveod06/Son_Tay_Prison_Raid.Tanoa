...
_cache= if (count _settings > 6) then {_settings select 6} else {false};
_createTask= if (count _settings > 7) then {_settings select 7} else {false};
...
if (_createTask) then
{
    ...
    {
        _sideX = _x;
        _taskNum = EOS_TASK_INDEX + 1;
        EOS_TASK_INDEX = EOS_TASK_INDEX + 1;
        _taskId = "EOS_Attack_" + str _taskNum;
        _description = ["Sweep and clear the area of all enemy units.", "Search and Destroy", _mkr];
        _destination = _mPos;
        _state = "CREATED";
        _priority = 0;
        ...
        [_sideX, _taskId, _description, _destination, _state, _priority, _showNotification, _type, _visibleIn3D] call BIS_fnc_taskCreate;
        diag_log format ["EOS TASK created task: %1 for side %2",_taskId,_sideX];
...
if (!(getmarkercolor _mkr == "colorblack")) then
{
	...
	for "_counter" from 1 to _aGrps do
    {
		...
		if (_aMin > 0) then
        {
            ...
            {
				0=[_mPos,units _aGroup,_mkrX,0,[0,20],true,true] spawn EOS_fnc_fillhouse; // FIXME spawn
			}
            else
            {
				0 = [_aGroup,_mkr] call EOS_fnc_taskpatrol; // FIXME spawn
			};
			_aGrp set [count _aGrp,_aGroup];
			0=[_aGroup,"INFskill"] call eos_fnc_setskill; // FIXME spawn
            if (_debug) then
            {
                PLAYER SIDECHAT (format ["Spawned House Patrol: %1",_counter]);
                0= [_mkr,_counter,"House Patrol",getpos (leader _aGroup)] call EOS_fnc_debug; // FIXME spawn
            };
	    };
	};
	...
	for "_counter" from 1 to _bGrps do
    {
		...
		if (_bMin > 0) then
        {	
			_pos = [_mkr,true] call EOS_fnc_SHK_pos;			
			_bGroup=[_pos,_bSize,_faction,_side] call EOS_fnc_spawngroup; // "scripts\eos\functions\infantry_fnc.sqf";
			0 = [_bGroup,_mkr] call EOS_fnc_taskpatrol; // FIXME spawn
			_bGrp set [count _bGrp,_bGroup];
			0=[_bGroup,"INFskill"] call eos_fnc_setskill; // FIXME spawn
            if (_debug) then 
            {
                PLAYER SIDECHAT (format ["Spawned Patrol: %1",_counter]);
                0= [_mkr,_counter,"patrol",getpos (leader _bGroup)] call EOS_fnc_debug; // FIXME spawn
            };
		};
	};	
    ...
    {	
        ...
        {
	    	0=[(_cGroup select 0),_cSize,(_cGroup select 2),_faction,_cargoType] call eos_fnc_setcargo; // FIXME spawn
	    };
	    					
	    0=[(_cGroup select 2),"LIGskill"] call eos_fnc_setskill; // FIXME spawn
	    0 = [(_cGroup select 2),_mkr] call EOS_fnc_taskpatrol; // FIXME spawn
	    _cGrp set [count _cGrp,_cGroup];			
	    							
        if (_debug) then
        {
            player sidechat format ["Light Vehicle:%1 - r%2",_counter,_cGrps];
            0= [_mkr,_counter,"Light Veh",(getpos leader (_cGroup select 2))] call EOS_fnc_debug; // FIXME spawn
        };
	};	
	...
	for "_counter" from 1 to _dGrps do
    {
		...
		0=[(_dGroup select 2),"ARMskill"] call eos_fnc_setskill; // FIXME spawn
		0 = [(_dGroup select 2),_mkr] call EOS_fnc_taskpatrol; // FIXME spawn
		_dGrp set [count _dGrp,_dGroup];
							
        if (_debug) then
        {
            player sidechat format ["Armoured:%1 - r%2",_counter,_dGrps];
            0= [_mkr,_counter,"Armour",(getpos leader (_dGroup select 2))] call EOS_fnc_debug; // FIXME spawn
        };
	};
	...
	for "_counter" from 1 to _eGrps do
    {
		...
		0=[(_eGroup select 2),"STAskill"] call eos_fnc_setskill; // FIXME spawn
		_eGrp set [count _eGrp,_eGroup];
							
        if (_debug) then
        {
            player sidechat format ["Static:%1",_counter];
            0= [_mkr,_counter,"Static",(getpos leader (_eGroup select 2))] call EOS_fnc_debug; // FIXME spawn
        };
	};	
	...
	for "_counter" from 1 to _fGrps do
    {
		...
		0=[(_fGroup select 2),"AIRskill"] call eos_fnc_setskill; // FIXME spawn
			
        if (_debug) then
        {
            player sidechat format ["Chopper:%1",_counter];
            0= [_mkr,_counter,"Chopper",(getpos leader (_fGroup select 2))] call EOS_fnc_debug; // FIXME spawn
        };
	};	
    	...
    	if (triggeractivated _clear and triggeractivated _taken and !_civZone) exitwith 
    	{
    		...
    		while {triggeractivated _eosActivated AND !(getmarkercolor _mkr == "colorblack")} do 
    		{
    			if (!triggeractivated _clear) then
    			{
                    ...
                    {
                        _taskIdX = _x;
                        [_taskIdX,"FAILED"] spawn BIS_fnc_taskSetState; // FIXME spawn
                    } forEach _taskIds;
    			}
                else
                {
                    ...
                    {
                        _taskIdX = _x;
                        [_taskIdX,"SUCCEEDED"] spawn BIS_fnc_taskSetState; // FIXME spawn
                    } forEach _taskIds;
    			};
    			sleep 3.0;
            };
