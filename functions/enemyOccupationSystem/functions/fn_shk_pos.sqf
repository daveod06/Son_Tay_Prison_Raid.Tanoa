/*
  EOS_fnc_SHK_pos - Random position generator for Arma 2
  ...
  Usage examples:
    myPos = ["markerName"] EOS_fnc_SHK_pos;
    myPos = ["myMarker1",true] EOS_fnc_SHK_pos;
  ...
  Usage examples:
    myPos = [getpos player,random 360,200,true] EOS_fnc_SHK_pos;
    myPos = [getmarkerpos "myMarker",125,random 500] EOS_fnc_SHK_pos;
    myPos = [getpos player,random 360,[200,500],false,2] EOS_fnc_SHK_pos;
  
  Example of creating multiple positions:
    for "_i" from 0 to 500 do {
      _p = [getpos player,random 360,random 1000] call EOS_fnc_SHK_pos;
      if (count _p > 0) then {
        call compile format ["
